...
class SolveWithJax:
    ...
    def misfit_sigma_jax(self, sigma_vec):
        ...
        def dtn_map(K_asm):
            ...
            sol = jax.ops.index_add(sol, np.index_exp[vol_idx,:], U_vol)
...
class SparseSolver:
  def __init__(self, iK, jK, v_h):
    self.iK = iK 
    self.jK = jK
    self.v_h = v_h
    self.ndof = v_h.mesh.n_p
    self.nElems = v_h.mesh.n_t
  ...
  def initSolver(self):
    ...
    def solveKuf(Kelem):
      ...
      bdy_data = np.eye(bdy_idx.shape[0])
      K_rhs = self.deleterowcol(K_ref,bdy_idx,vol_idx).tocoo()
      K_rhs = K_rhs*bdy_data;
      K_rhs = cvxopt.spmatrix(K_rhs.data.astype(np.double),K_rhs.row.astype(np.int),K_rhs.col.astype(np.int))

      B = cvxopt.matrix(K_rhs)
...
class SolveWithJaxSparse:
    def __init__(self, v_h, Data):
        ...
        self.v_h = v_h
        self.Data = Data
        self.K0, self.iK, self.jK = self.computeK0();
        self.Solver = SparseSolver(self.iK, self.jK, self.v_h)
    ...
    def computeK0(self):
        ...
        for e in range(self.v_h.mesh.n_t):  # integration over one triangular element at a time
            ...
            idx_j[e,:] = (np.ones((3,1))*nodes).reshape((9,))
            K0[e,:] = S_local.reshape((9,))
        return K0, idx_i.reshape((-1)), idx_j.reshape((-1))
    ...
    def misfit_sigma_jax(self, sigma_vec):
        ...
        def stiffness_matrix(sigma_vec):
            ...
            '''
            K_asm = jnp.zeros((self.v_h.mesh.n_p, self.v_h.mesh.n_p)) 
            K_elem = jnp.einsum('i,ij-> ij', sigma_vec, self.K0).flatten() 
            K_asm = jax.ops.index_add(K_asm, jax.ops.index[self.iK, self.jK], K_elem) 
            return K_asm
        #-----------------------#
        def dtn_map(K_asm):
            '''
            ...
            U_vol = self.Solver.solve(K_asm) # 301, 61                
            sol = jnp.zeros((n_pts,n_bdy_pts)) # 362,61
